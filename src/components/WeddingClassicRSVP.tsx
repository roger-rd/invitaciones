...
interface WeddingClassicRSVPProps extends WeddingRsvpData {
...
export default function WeddingClassicRSVP({ whatsappNumber, brideAndGroom, deadline, notes }: WeddingClassicRSVPProps) {
  const helpId = useId();
  ...
  const deadlineLabel = deadlineDate && Number.isFinite(deadlineDate.getTime())
    ...
    : undefined;
  const fieldClass = "mt-2 min-h-12 w-full rounded-lg border border-classic-gold/50 bg-classic-cream px-4 py-3 text-base focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-classic-ink";
  ...
  return (
    <section aria-label="Confirmación de asistencia" className="bg-classic-ivory px-6 py-20 text-classic-ink sm:py-28">
      <div className="mx-auto max-w-xl">
        <h2 className="text-center font-display-classic text-3xl sm:text-4xl">¿Nos acompañas?</h2>
        ...
        <form onSubmit={submit} onChange={() => setPreparedUrl("")} aria-describedby={helpId} className="mt-10 grid gap-6">
          ...
          <button type="submit" className="min-h-12 cursor-pointer rounded-full bg-classic-ink px-6 py-3 text-classic-cream hover:bg-classic-ink/90 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-classic-ink">Confirmar por WhatsApp</button>
