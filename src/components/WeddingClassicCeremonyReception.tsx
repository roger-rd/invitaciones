...
interface WeddingClassicCeremonyReceptionProps {
...
function VenueCard({ label, venue }: { label: string; venue: WeddingVenue }) {
  ...
  const linkClass = "inline-flex min-h-12 max-w-full items-center justify-center gap-2 rounded-full border border-classic-gold/50 px-5 py-3 text-sm hover:bg-classic-sage-soft/50 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-classic-ink";

  return (
    <article className="min-w-0 border border-classic-gold/25 bg-classic-cream p-6 text-center [overflow-wrap:anywhere] sm:p-10">
      <h3 className="text-xs uppercase tracking-[.2em]">{label}</h3>
      <p className="mt-6 font-display-classic text-3xl leading-tight">{venue.name}</p>
      <p className="mt-4 whitespace-pre-line text-base leading-relaxed">{venue.address}</p>
      <p className="mt-6 flex items-center justify-center gap-2 font-display-classic text-xl">
        <Clock aria-hidden="true" focusable="false" className="h-5 w-5 shrink-0 text-classic-gold" />
...
export default function WeddingClassicCeremonyReception({ ceremony, reception }: WeddingClassicCeremonyReceptionProps) {
  if (!ceremony && !reception) return null;

  return (
    <section aria-label="Ceremonia y recepción" className="bg-classic-ivory px-6 py-20 text-classic-ink sm:py-28">
      <Reveal>
        <h2 className="mb-12 text-center font-display-classic text-3xl sm:text-4xl">Dónde celebraremos</h2>
      </Reveal>
      <div className={`mx-auto grid gap-8 ${ceremony && reception ? "max-w-6xl md:grid-cols-2" : "max-w-xl"}`}>
        {ceremony && <Reveal className="min-w-0"><VenueCard label="Ceremonia" venue={ceremony} /></Reveal>}
        {reception && <Reveal className="min-w-0"><VenueCard label="Recepción" venue={reception} /></Reveal>}
      </div>
    </section>
  );
}
