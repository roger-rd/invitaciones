...
interface WeddingDressCodeProps {
  dressCode?: WeddingDressCodeData;
}

export default function WeddingDressCode({ dressCode }: WeddingDressCodeProps) {
  ...
  return (
    <section aria-label="Código de vestimenta" className="bg-romantic-ivory px-6 py-20 text-center text-romantic-ink sm:py-28">
      <div className="mx-auto max-w-2xl [overflow-wrap:anywhere]">
        <p className="text-xs uppercase tracking-[.2em]">Código de vestimenta</p>
        <h2 className="mt-6 font-display-romantic text-4xl sm:text-5xl">{dressCode.formality}</h2>
        {groups.map(({ title, colors }) => colors?.length ? (
          <div key={title} className="mt-8">
            <h3 className="font-display-romantic text-xl">{title}</h3>
            <ul className="mt-4 flex flex-wrap justify-center gap-2">
              {colors.map((color, index) => <li key={`${color}-${index}`} className="max-w-full rounded-full border border-romantic-gold/30 bg-romantic-cream px-4 py-2 text-sm">{color}</li>)}
            </ul>
          </div>
        ) : null)}
