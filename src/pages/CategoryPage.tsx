...
import type { CategoryDefinition, CatalogExperienceType } from "../types/catalog";
...
function CategoryContent({ category }: { category: CategoryDefinition["id"] }) {
 const definition = categories.find(c => c.id === category)!;
 const hasExperienceTabs = category === "wedding" || category === "kids-birthday";
 const [experienceType, setExperienceType] = useState<CatalogExperienceType>(category === "kids-birthday" ? "digital" : "interactive");
 const panelId = useId();
 const models = catalogModels.filter(m => m.category === category && (!hasExperienceTabs || m.experienceType === experienceType));
